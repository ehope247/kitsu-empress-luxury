  ...
  return (
    <main className="min-h-screen font-sans selection:bg-gold selection:text-white overflow-x-hidden">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? "bg-cream/95 backdrop-blur-xl py-3 shadow-sm border-b border-gold/10" : "bg-transparent py-5"}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center">
          <div className="flex items-center gap-1.5 text-xl md:text-2xl font-serif font-bold tracking-widest text-charcoal">
            <Crown className="text-gold" size={20} /> $KITSU
          </div>
          ...
          <a href="https://t.me/Kitsucatonsol" target="_blank" rel="noopener noreferrer">
            <button className="bg-charcoal text-white px-5 md:px-8 py-2.5 md:py-3 text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-gold transition-all duration-300 shadow-lg">
              Join Pride
            </button>
          </a>
        </div>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 pt-20">
        ...
        >
          <div className="mb-6 md:mb-8 px-6 py-2 border border-charcoal/10 rounded-full bg-white/50 backdrop-blur-sm text-[9px] md:text-[10px] font-bold tracking-[0.3em] uppercase text-charcoal/70 shadow-sm">
            The Internet Belongs to Cats
          </div>

          <h1 className="text-5xl md:text-[9rem] font-serif font-bold tracking-tighter mb-6 text-charcoal leading-[1.1] md:leading-[0.9]">
            The <span className="text-gold-gradient italic pr-2 md:pr-4">Empress</span> <br />
            of Meme Coins.
          </h1>

          <p className="text-charcoal/60 text-base md:text-xl max-w-2xl mx-auto mb-10 md:mb-12 font-light leading-relaxed px-4">
            Built for cat lovers. Owned by believers. <br className="hidden md:block" />
            Dogs compete. The Empress simply reigns.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 md:gap-5 w-full sm:w-auto px-10 sm:px-0">
            <button 
              onClick={handleBuyClick}
              className="bg-gold text-white px-12 py-4 md:py-5 text-xs font-bold uppercase tracking-[0.2em] hover:bg-charcoal hover:scale-105 transition-all duration-300 shadow-xl shadow-gold/30 min-w-[200px]"
            >
              {buyText}
            </button>
            <a href="#lore">
              <button className="w-full group border border-charcoal/20 px-12 py-4 md:py-5 text-xs font-bold uppercase tracking-[0.2em] hover:border-gold hover:text-gold transition-all duration-300 flex items-center justify-center gap-2 bg-white/50 backdrop-blur-sm">
        ...
        <motion.div 
          ...
          className="mt-16 md:mt-20 relative w-64 h-64 md:w-96 md:h-96 rounded-full border border-gold/20 bg-gradient-to-b from-cream-dark to-white flex items-center justify-center luxury-shadow overflow-hidden"
        >
      ...
      <section id="lore" className="py-24 md:py-32 px-6 bg-white relative overflow-hidden">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 md:gap-20 items-center">
          <motion.div 
            ...
            variants={fadeInUp}
          >
            <div className="flex items-center gap-2 text-gold font-bold tracking-[0.2em] text-[10px] md:text-xs uppercase mb-6">
              <ScrollText size={16} /> The Rise of Kitsu
            </div>
            <h2 className="text-4xl md:text-6xl font-serif font-bold mb-8 text-charcoal leading-tight">
              Dogs bark. <br />
              <span className="text-gold italic">Kitsu observes.</span>
            </h2>
            <p className="text-charcoal/70 mb-6 text-base md:text-lg font-light leading-relaxed">
      ...
      {/* --- 4. TOKENOMICS --- */}
      <section id="tokenomics" className="py-24 md:py-32 bg-charcoal text-cream relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          >
            <h2 className="text-gold text-[10px] font-bold tracking-[0.3em] uppercase mb-4">Financial Architecture</h2>
            <h3 className="text-4xl md:text-7xl font-serif mb-6">Luxury Doesn't Tax Loyalty.</h3>
            <p className="text-cream/50 max-w-2xl mx-auto font-light text-sm md:text-base">
              A fair launch. No insider allocations. Pure, decentralized elegance.
            </p>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            <div className="bg-white/5 border border-white/10 p-8 md:p-10 backdrop-blur-md text-center group hover:border-gold/50 transition-all duration-500">
              <Gem className="text-gold mx-auto mb-6" size={32} />
              <h4 className="text-3xl md:text-4xl font-serif text-white mb-2">1 Billion</h4>
              <p className="text-gold text-[10px] uppercase tracking-widest mb-4">Total Supply</p>
              <p className="text-cream/40 text-xs md:text-sm font-light">Fixed supply. No minting function.</p>
            </div>

            <div className="bg-white/5 border border-white/10 p-8 md:p-10 backdrop-blur-md text-center group hover:border-gold/50 transition-all duration-500">
              <ShieldCheck className="text-gold mx-auto mb-6" size={32} />
              <h4 className="text-3xl md:text-4xl font-serif text-white mb-2">0% / 0%</h4>
              <p className="text-gold text-[10px] uppercase tracking-widest mb-4">Buy & Sell Tax</p>
              <p className="text-cream/40 text-xs md:text-sm font-light">We do not tax our community.</p>
            </div>

            <div className="bg-white/5 border border-white/10 p-8 md:p-10 backdrop-blur-md text-center group hover:border-gold/50 transition-all duration-500">
              <Rocket className="text-gold mx-auto mb-6" size={32} />
              <h4 className="text-3xl md:text-4xl font-serif text-white mb-2">Burned</h4>
              <p className="text-gold text-[10px] uppercase tracking-widest mb-4">Liquidity Pool</p>
              <p className="text-cream/40 text-xs md:text-sm font-light">100% of LP tokens burned at launch.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="roadmap" className="py-24 md:py-32 px-6 bg-cream-dark relative">
        <div className="max-w-5xl mx-auto">
          <motion.div 
            ...
            className="text-center mb-16 md:mb-20"
          >
            <h2 className="text-gold text-[10px] font-bold tracking-[0.3em] uppercase mb-4">The Dynasty</h2>
      ...
      {/* --- 6. FOOTER --- */}
      <footer className="bg-charcoal text-white py-16 md:py-20 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col items-center text-center">
          <Crown className="text-gold mb-6" size={40} />
          <h2 className="text-3xl md:text-6xl font-serif mb-8 px-4 leading-tight">Ready to Rule?</h2>
          <p className="text-cream/50 max-w-xl mb-10 text-base md:text-lg font-light">
            The throne is waiting. Join the most exclusive community in crypto.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6 mb-16 w-full sm:w-auto px-10 sm:px-0">
             <a href="https://x.com/Kitsucatonsol" target="_blank" rel="noopener noreferrer" className="w-full">
                <button className="w-full bg-gold text-charcoal px-10 py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-white transition-all">
                    Twitter / X
                </button>
             </a>
             <a href="https://t.me/Kitsucatonsol" target="_blank" rel="noopener noreferrer" className="w-full">
                <button className="w-full border border-white/20 text-white px-10 py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:border-gold hover:text-gold transition-all">
                    Telegram
                </button>
             </a>
          </div>
